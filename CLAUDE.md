...
기능 구현 후 검증·디버깅까지 완료한 시점에 반드시 다음을 수행한다:

1. [`docs/GAME_DESIGN.md`](docs/GAME_DESIGN.md) 섹션 7을 확인한다.
2. 해당 항목이 있으면 → `[ ]`를 `[x]`로 바꾸고 날짜를 추가한다.  
   예: `- [x] **기능 이름** — 설명 ✅ YYYY-MM-DD`
3. 해당 항목이 없으면 → 적절한 우선순위 범주에 항목을 추가하고 바로 `[x]`로 표시한다.

이 규칙은 Claude Code가 코드 작업을 할 때마다 적용한다. 잊지 말 것.
